%%%%%%%%%%%%%%%%%
%Nome: Nicolas P. Lane
%Data: 13/10/2015
%Dúvidas ou contribuições: nicolaspeterlane@hotmail.com
%%%%%%%%%%%%%%%%%

\NeedsTeXFormat{LaTeX2e}
%-----------------------------------------------------------
\documentclass[a4paper,12pt]{monografia}
\usepackage{amsmath, amsthm, amsfonts, amssymb}
\usepackage[english, brazilian]{babel}
\usepackage[titletoc]{appendix}
\usepackage[latin1]{inputenc}
\usepackage[mathcal]{eucal}
\usepackage[alf]{abntcite}
\usepackage{subfigure}
\usepackage{url}
\usepackage{isoaccent}
\usepackage{textcase}
\usepackage{multirow}
\usepackage{latexsym}
\usepackage{graphicx}
\usepackage{listings}
\usepackage{acronym}
\usepackage{bm}

\usepackage{stmaryrd}

\usepackage{pgf,tikz}
\usepackage{tkz-graph}  
\usetikzlibrary{shapes.geometric}%   

\usepackage{csquotes}

\lstset{basicstyle=\tiny, language=C++}
\makeindex
%-----------------------------------------------------------Começa documento
\begin{document}
%----------------- Título e Dados do Autor -----------------
\titulo{Analise do desempenho do Mininet em relação a redes tradicionais na transmissão de fluxo de dados de tamanho médio}
\autor{Renan S. Silva, Ricardo Sohn, Ruan P. Medeiros}
\nome{}
\ultimonome{}

%---------- Informe o Curso e Grau -----
\bacharelado \curso{Ci\^encia da Computa\c{c}\~ao} \mes{Junho} \ano{2015} 
\data{\today} % data da aprovação
\cidade{Joinville}

%----------Informações sobre a Institução -----------------
\instituicao{Universidade do Estado de Santa Catarina}
\sigla{UDESC} \unidadeacademica{Centro de Ci\^encias Tecnol\'ogicas}

%------Nomes do Orientador, 1o. Examinador e 2o. Examinador-
\orientador{Charles Christian Miers}
\examinadorum{Charles Christian Miers}
\examinadordois{Charles Christian Miers}
%\examinadorquatro{Nome do Examinador 4}

%--------- Títulos do Orientador 1o. e 2o. Examinadores ----
\ttorientador{Doutor}
\ttexaminadorum{Doutor}
\ttexaminadordois{Doutor}
%\ttexaminadorquatro{Título do Examinador 4}

\maketitle
%---------------------- AGRADECIMENTO --------------
\agradecimento{Agradecimentos}



\newpage
%---------------------- EPÍGRAFE --------------
\begin{epigrafe}

  ``We must know - we will know!''\\ - David Hilbert

\end{epigrafe}
%--------Digite aqui o seu resumo em Português--------------
\resumo{Resumo}
\input{resumo}%chamada de arquivo resumo.tex

\noindent Palavras-chaves: SDN, Openflow, Mininet, software livre, transferência
%-----------Digite aqui o seu resumo em Inglês--------------
\resumo{Abstract}
\input{abstract}     %chamada de arquivo abstract.tex

\noindent Keywords: SDN, Openflow, Mininet, open source, trasnference
%-----------------------------------------------------------

\listoffigures       %gera página com lista de figuras a partir de todas as entradas feitas no documento

% lista de abreviações
\listoftables        %gera página com lista de tabelas a partir de todas as entradas feitas no documento
\input{acronimos}    %chamada de arquivo acronimos.tex

%----Sumário, lista de figura e de tabela ------------
\tableofcontents      %gera página com sumário a partir de todas as entradas feitas no documento

\newpage

%--------------Início do Conteúdo---------------------------
\pagestyle{ruledheader} %estilo abnt2
\input{introducao}     %chamada de arquivo introducao.tex
\input{capitulos}      %chamada de arquivo capitulos.tex (desenvolvimento de seu TCC efetivamente)

%--------------Bibliografia e apêndices--------------------
\bibliographystyle{abnt-alf}
\bibliography{bibliografia}
\input{anexo}		%chamada de arquivo anexo.tex
\input{apendice}	%chamada de arquivo apendice.tex
\end{document}
