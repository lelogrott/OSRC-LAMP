...
set output 'taxa_compare.png'

set xlabel "Tempo (s)"
set autoscale

set ylabel "Taxa (Kbps)"
set title "Taxa de transferencia no Ambiente OpenStack "
...
plot "1024_log"        using (($0) * 1 + 1):1 title "1024 Mb"          smooth sbezier  with lines ls 1, \
     "768_log"         using (($0) * 1 + 1):1 title "768 Mb"           smooth sbezier  with lines ls 2, \
     "512_log"         using (($0) * 1 + 1):1 title "512 Mb"           smooth sbezier  with lines ls 3, \
     "256_log"         using (($0) * 1 + 1):1 title "256 Mb"           smooth sbezier  with lines ls 4
