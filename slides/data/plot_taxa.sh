...
set title "Comparativo de velocidade de transferencia"
...
f(x) = mean_y1
fit f(x) 'data_cp_mininet/times' u 1:3 via mean_y1
stddev_y1 = sqrt(FIT_WSSR / (FIT_NDF + 1 ))

g(x) = mean_y2
fit g(x) 'data_cp_real/times' u 1:3 via mean_y2
...
plot "data_cp_mininet/times"        using 1:3   title 'Mininet'         with linespoints ls 1, \
     "data_cp_real/times"           using 1:3   title 'Real'            with linespoints ls 2, \
      mean_y1                                   title 'Média Mininet'   with lines       ls 7, \
      mean_y2                                   title 'Média Real'      with lines       ls 8
#     mean_y1 w l ls 4, mean_y1+stddev_y1 w l ls 3, mean_y1-stddev_y1 w l ls 3, \
#     mean_y2 w l ls 5, mean_y2+stddev_y2 w l ls 6, mean_y2-stddev_y2 w l ls 5
#
