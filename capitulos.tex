...
Na Fig.~\ref{xsser} têm-se um exemplo do funcionamento da ferramenta XSSer.
...
utilizado algumas ferramentas auxiliares, como: Vega, sqlmap (ver seção~\ref{lampkali} para mais detalhes).

\section{AMBIENTE DE TESTES E PLANO DE TESTES}

Todos os testes aqui apresentados foram executados em \textit{sites} próprios para testes de penetração, ou seja, são \textit{sites} próprios para que seja feita os devidos 
testes e com isso aprimorar habilidades sem nenhum tipo de dano. 

Também foi realizado os testes com auxílio de ferramentas específicas para cada tipo de ataque comentado na subseção \ref{principaisataques}. 


\section{SESSÃO DE ATAQUES}

As Fig~\ref{fig:vega_hack} e \ref{fig:vega_acuart} representam as vulnerabilidades encontradas quando utilizado do Vega Scan (ver subseção \ref{ferramentas} para mais detalhes).
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{imagens/sqlmap_columnsnames_acuart.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{imagens/sqlmap_tablenames2_acuart.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{imagens/sqlmap_tablenames_acuart.png}
