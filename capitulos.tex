...
que são usados para implementar uma solução. O conjunto de soluções LAMP é provavelmente
o mais comum usado para hostear páginas e aplicações web. Especialmente entre pequenas e
médias empresas, pois os principais componentes são de código aberto e não têm custo.
*Vamos ver como uma web request* funciona para ver o que cada componente faz. Um visitante
entra em um site pelo seu browser, que irá enviar um pedido para um servidor web que, por
sua vez, irá executar alguns comandos e devolver para o browser um arquivo em HTML. Todos os
componentes LAMP estão instalados e sendo executados nesse servidor web. Um servidor web é um
tipo de computador, logo ele necessita de um sistema operacional. O "L" em LAMP *stands for*
Linux, que é o sistema operacional nesse conjunto de soluções. O sistema operacional é a base
na qual todos os outros componentes podem ser executados. O "A" *stands for* Apache, que é o
software de servidor web. Ele recebe e manipula todos os pedidos do visitante. Se o visitante
*requests* um arquivo ou uma página no site, o Apache procura no servidor pelo o arquivo e
envia devolta para o browser do visitante. O "P" geralmente *stands for* PHP que também pode
ser Perl ou Python, mas isso não é muito comum. PHP é uma linguagem de programação que é utilizada 
para gerar páginas dinâmicas. Quando um visitante *requests* um arquivo em PHP o Apache não apenas 
pega esse arquivo e envia de volta para o browser como ele faz com outros tipos de arquivos.
Em vez disso, ele primeiro envia o arquivo para o PHP, que execute o código no arquivo e gera
um arquivo HTML para a página web. O HTML gerado pelo PHP volta para o Apache e depois para o
browser do visitante. O 'M' *stands for* MySQL que é um SGBD usado para armazenar as informações
de um site. Frequentemente os dados necessários para gerar o arquivo PHP depende em quem o
visitante é. Como o nome dele, número da conta ou histórico de pedidos. O banco de dados permite
que essas informações fiquem armazenadas.
