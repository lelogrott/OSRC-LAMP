...
de não ser muito comum (mais detalhes serão vistos no tópico \ref{sec:componentes}).

Uma exemplo do funcionamento de cada componente do \ac{LAMP} em um servidor \textit{web} se dá por
um visitante entrando em um \textit{site} pelo seu navegador, o qual irá enviar um pedido para o servidor,
onde todos os componentes \ac{LAMP} estão instalados e sendo executados. Este servidor pode então prover ao usuário, interação com 
...
1995 quando a \ac{CERN} httpd introduziu o conceito de \ac{CGI}, que tornou possível aos servidores
executar códigos para criar páginas dinâmicas. Linux, \ac{CERN} httpd e linguagens de programação do lado do servidor
